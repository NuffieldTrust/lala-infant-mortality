...
npages_dist <- ggforce::n_pages(ggplot(infant_mortality_vars_long, aes(x = as.factor(year_num), y = value)) +
                                  ggforce::facet_wrap_paginate(~ variable_new, ncol = 2, nrow = 3, scales = "free_y"))

pdf("output/descriptives_independent/plots/Dist_vars_infant_mortality_utla.pdf")

for(i in 1:npages_dist){
  
  print(
    ggplot(infant_mortality_vars_long %>%
      ...
      ggforce::facet_wrap_paginate(~ variable_new, ncol = 2, nrow = 3, scales = "free_y", page = i) +
      labs(x = "Year"
           , title = "Local authority distribution of infant mortality variables by year"
...
infant_mortality_desc <- as_tibble(psych::describeBy(infant_mortality_vars_long$value, group = infant_mortality_vars_long$variable
...
write_csv(infant_mortality_desc,"output/descriptives_independent/Summary_vars_infant_mortality_utla.csv")


# Identify missing values -------------------------------------------------

infant_mortality_missing <- infant_mortality_vars_long %>%
  group_by(variable, variable_new, year_num) %>%
  summarise(missing = sum(is.na(value))) %>%
  filter(missing > 0)

write_csv(infant_mortality_missing, "output/descriptives_independent/Missing_vars_infant_mortality_utla.csv")

infant_mortality_missing_la <- infant_mortality_vars_long %>%
  filter(is.na(value)) %>%
  select(utla17cd, variable, variable_new, year_num, year_label)

write_csv(infant_mortality_missing_la, "output/descriptives_independent/Missing_la_vars_infant_mortality_utla.csv")
...
infant_mortality_desc_limits <- infant_mortality_desc %>%
  select(variable, mean, sd, Q0.25, Q0.75, IQR) %>%
...
infant_mortality_vars_long_stat <- infant_mortality_vars_long %>%
  left_join(infant_mortality_desc_limits, by = "variable") %>%
...
infant_mortality_outliers <- infant_mortality_vars_long_stat %>%
  group_by(variable) %>%
...
write_csv(infant_mortality_outliers,"output/descriptives_independent/Outliers_vars_infant_mortality_utla.csv")

infant_mortality_outliers_la <- infant_mortality_vars_long_stat %>%
  filter(sd_outlier_tot_3 == 1 | iqr_outlier_tot_1.5 == 1) %>%
  select(year_num, utla17cd, variable
         , iqr_outlier_tot_1.5
...
write_csv(infant_mortality_outliers_la,"output/descriptives_independent/Outliers_la_vars_infant_mortality_utla.csv")


# Detect variability in longitudinal data ---------------------------------

## Method 1
#	At each time point, compare the average value across all local authorities to each individual local authority value

m1_infant_mortality_se_avg <- infant_mortality_vars_long %>%
  group_by(variable_new, year_num) %>%
  summarise(avg_value = mean(value, na.rm = TRUE))

m1_infant_mortality_vars_long_avg <- infant_mortality_vars_long %>%
  left_join(m1_infant_mortality_se_avg
            , by = c("variable_new", "year_num")) %>%
  mutate(diff = avg_value - value) 

m1_infant_mortality_vars_avg_diff <- m1_infant_mortality_vars_long_avg %>%
...
m1_variability_limits <- m1_infant_mortality_vars_avg_diff %>%
...
npages_dist_var_m1 <- ggforce::n_pages(ggplot(m1_infant_mortality_vars_avg_diff, aes(x = tidytext::reorder_within(utla17cd,sd_diff,variable_new), y = sd_diff)) +
                                         geom_point(size = 0.2) +
                                         ...
                                         ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2))

pdf("output/descriptives_independent/plots/Variability_m1_sd_diff_vars_infant_mortality_utla.pdf")

for(i in 1:npages_dist_var_m1){

print(ggplot(m1_infant_mortality_vars_avg_diff, aes(x = tidytext::reorder_within(utla17cd,sd_diff,variable_new), y = sd_diff)) +
        geom_point(size = 0.2) +
        geom_hline(data = m1_variability_limits, mapping = aes(yintercept = high1.5), colour = NT_colour("bright blue")) +
        geom_hline(data = m1_variability_limits, mapping = aes(yintercept = high3), colour = NT_colour("bright red")) +
        geom_hline(data = m1_variability_limits, mapping = aes(yintercept = low1.5), colour = NT_colour("bright blue")) +
        geom_hline(data = m1_variability_limits, mapping = aes(yintercept = low3), colour = NT_colour("bright red")) +
        ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2, page = i) +
...
npages_box_var_m1 <- ggforce::n_pages(ggplot(m1_infant_mortality_vars_avg_diff, aes(y = sd_diff)) +
                                        stat_boxplot(geom = "errorbar", width = 0.2) +
                                        geom_boxplot(outlier.colour = NT_colour("bright red"), outlier.shape = 8) +
                                        ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2))

pdf("output/descriptives_independent/plots/Variability_m1_box_vars_infant_mortality_utla.pdf")

for(i in 1:npages_box_var_m1){

print(ggplot(m1_infant_mortality_vars_avg_diff, aes(y = sd_diff)) +
        stat_boxplot(geom = "errorbar", width = 0.2) +
        geom_boxplot(outlier.colour = NT_colour("bright red"), outlier.shape = 8) +
        ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2, page = i) +
...
m1_variability_outliers_summary <- m1_infant_mortality_vars_avg_diff %>%
...
write_csv(m1_variability_outliers_summary, "output/descriptives_independent/Variability_outliers_m1_vars_infant_mortality_utla.csv")

m1_variability_la_outliers_summary <- m1_infant_mortality_vars_avg_diff %>%
  left_join(m1_variability_limits, by = "variable_new") %>%
  mutate(outlier_1.5 = case_when(sd_diff > high1.5 ~ 1
                                 , TRUE ~ 0)
         , outlier_3 = case_when(sd_diff > high3 ~ 1
  ...
  select(variable_new, utla17cd, outlier_1.5, outlier_3)

write_csv(m1_variability_la_outliers_summary, "output/descriptives_independent/Variability_outliers_m1_la_vars_infant_mortality_utla.csv")

m1_outliers_la_plot <- m1_infant_mortality_vars_long_avg %>%
...
pdf("output/descriptives_independent/plots/Variability_outliers_m1_la_vars_infant_mortality_utla.pdf")

for(i in m1_ind_list_plot){

print(ggplot(m1_outliers_la_plot %>%
        ...
        geom_line(data = m1_infant_mortality_se_avg %>%
...
m2_missing_prep <- infant_mortality_vars_long %>%
  filter(is.na(value)) %>%
  select(variable_new)
...
m2_fitted_models <- infant_mortality_vars_long %>%
...
m2_infant_mortality_vars_long_avg <- infant_mortality_vars_long %>%
  left_join(m2_fitted_models
            , by = c("variable_new", "utla17cd")) %>%
  mutate(fit = a + (b * year_num)
         , diff = value - fit) 

m2_infant_mortality_vars_avg_diff <- m2_infant_mortality_vars_long_avg %>%
  group_by(variable_new, utla17cd) %>%
  summarise(total_diff = sum(diff, na.rm = TRUE)
            , avg_diff = mean(diff, na.rm = TRUE)
            , sd_diff = sd(diff, na.rm = TRUE)) %>%
...
m2_variability_limits <- m2_infant_mortality_vars_avg_diff %>%
  group_by(variable_new) %>%
  summarise(q25 = quantile(sd_diff, 0.25, na.rm = TRUE)
            , q75 = quantile(sd_diff, 0.75, na.rm = TRUE)
            , iqr = q75-q25
...
npages_dist_var_m2 <- ggforce::n_pages(ggplot(m2_infant_mortality_vars_avg_diff, aes(x = tidytext::reorder_within(utla17cd,sd_diff,variable_new), y = sd_diff)) +
                                         geom_point(size = 0.2) +
...
pdf("output/descriptives_independent/plots/Variability_m2_sd_diff_vars_infant_mortality_utla.pdf")

for(i in 1:npages_dist_var_m2){
  
  print(ggplot(m2_infant_mortality_vars_avg_diff, aes(x = tidytext::reorder_within(utla17cd,sd_diff,variable_new), y = sd_diff)) +
          geom_point(size = 0.2) +
          geom_hline(data = m2_variability_limits, mapping = aes(yintercept = high1.5), colour = NT_colour("bright blue")) +
          geom_hline(data = m2_variability_limits, mapping = aes(yintercept = high3), colour = NT_colour("bright red")) +
          geom_hline(data = m2_variability_limits, mapping = aes(yintercept = low1.5), colour = NT_colour("bright blue")) +
...
npages_box_var_m2 <- ggforce::n_pages(ggplot(m2_infant_mortality_vars_avg_diff, aes(y = sd_diff)) +
                                        stat_boxplot(geom = "errorbar", width = 0.2) +
                                        geom_boxplot(outlier.colour = NT_colour("bright red"), outlier.shape = 8) +
                                        ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2))

pdf("output/descriptives_independent/plots/Variability_m2_box_vars_infant_mortality_utla.pdf")

for(i in 1:npages_box_var_m2){
  
  print(ggplot(m2_infant_mortality_vars_avg_diff, aes(y = sd_diff)) +
          stat_boxplot(geom = "errorbar", width = 0.2) +
          geom_boxplot(outlier.colour = NT_colour("bright red"), outlier.shape = 8) +
          ggforce::facet_wrap_paginate(~ variable_new, scales = "free", nrow = 3, ncol = 2, page = i) +
          labs(y = "Standard deviation of difference"
...
m2_variability_outliers_summary <- m2_infant_mortality_vars_avg_diff %>%
...
write_csv(m2_variability_outliers_summary, "output/descriptives_independent/Variability_outliers_m2_vars_infant_mortality_utla.csv")

m2_variability_la_outliers_summary <- m2_infant_mortality_vars_avg_diff %>%
  left_join(m2_variability_limits, by = "variable_new") %>%
  mutate(outlier_1.5 = case_when(sd_diff > high1.5 ~ 1
                                 , TRUE ~ 0)
         , outlier_3 = case_when(sd_diff > high3 ~ 1
...
write_csv(m2_variability_la_outliers_summary, "output/descriptives_independent/Variability_outliers_m2_la_vars_infant_mortality_utla.csv")

m2_outliers_la_plot <- m2_infant_mortality_vars_long_avg %>%
...
pdf("output/descriptives_independent/plots/Variability_outliers_m2_la_vars_infant_mortality_utla.pdf")
...
for(i in m2_ind_list_plot){
  ...
  print(ggplot(m2_outliers_la_plot %>%
          ...
          geom_line(aes(x = factor(year_num), y = value, group = utla17cd, colour = utla17cd)) +
          # geom_line(data = m1_infant_mortality_se_avg %>%
          #             filter(variable_new == i), mapping = aes(x = factor(year_num), y = avg_value, group = variable_new), colour = "black", lty = "dashed") +
          labs(x = "Year"
               , title = i) +
          scale_colour_manual("Local authority"
...
npages_norm_var <- ggforce::n_pages(ggplot(infant_mortality_vars_long %>%
                                             left_join(infant_mortality_desc %>%
                                                         select(variable_new, year_char, mean, skew)
                                      ...
                                      ggforce::facet_wrap_paginate( ~ variable, scales = "free_x", ncol = 3, nrow = 5))

pdf("output/descriptives_independent/plots/Normality_vars_infant_mortality_utla.pdf")

for(i in 1:npages_norm_var){

print(ggplot(infant_mortality_vars_long %>%
               left_join(infant_mortality_desc %>%
                           select(variable_new, year_char, mean, skew)
                         , by = c("variable_new", "year_char"))
             , aes(x = value)) +
        ...
        geom_text(infant_mortality_desc
                  ...
                  , hjust = "inward", vjust = "inward"
                  , size = 3, colour = NT_colour("NT ink")) +
        ggforce::facet_wrap_paginate( ~ variable, scales = "free", ncol = 3, nrow = 5, page = i) +
        labs(y = "Frequency"
             , title = "Density Plot") +
...
for(i in colnames(infant_mortality_vars %>%
                  select(-"utla17cd"))){
  
  pearson_correlation <- plyr::ddply(infant_mortality_vars_long %>% 
                                       filter(variable != i) %>% 
                                       left_join(infant_mortality_vars_long %>% 
                                                   filter(variable == i) %>%
...
writexl::write_xlsx(excel_sheets, "output/descriptives_independent/Correlation_all_vars_infant_mortality_UTLA.xlsx"
                    , col_names = TRUE)
